...
export class Jimp extends EventEmitter {
   ...
   */
  public static appendConstructorOption (name: string, test: Function, run: Function) {
   ...
   */
  public static read(...args: any[]): Promise<any> {
  ...
  public static create = Jimp.read;
 ...
 */
public static rgbaToInt (r, g, b, a, cb) {
  if (
    typeof r !== 'number' ||
    typeof g !== 'number' ||
    typeof b !== 'number' ||
 ...
 */
public static cssColorToHex (cssColor) {
 ...
 */
public static diff (img1: Jimp, img2: Jimp, threshold: number = 0.1): object {
 ...
 */
public static distance (img1, img2) {
 ...
 * @returns {number} a number ranging from 0 to 1, 0 means they are believed to be identical
 */
public static compareHashes (hash1, hash2) {
  const phash = new ImagePHash();
 ...
 */
public static colorDiff (rgba1, rgba2) {
  ...
  private _background = 0x00000000;

  // Default MIME is PNG
  private _originalMime = (Jimp as any).MIME_PNG;
  ...
  private _quality: any;
  private _deflateLevel: any;
  private _deflateStrategy: any;
   ...
   */
  getMIME() {
    const mime = this._originalMime || (Jimp as any).MIME_PNG;
   ...
   * @returns {Jimp} this for chaining of methods
   */
  getBase64(mime: string, cb: (arg0: Error, arg1: Jimp) => any): Jimp {
    if (mime === (Jimp as any).AUTO) {
   ...
   */
  getPixelIndex(x: number, y: number, edgeHandling?: string, cb?: Function): number {
    let xi;
    let yi;
    ...
    if (!edgeHandling) {
      edgeHandling = (Jimp as any).EDGE_EXTEND;
    }
    ...
    if (edgeHandling === (Jimp as any).EDGE_EXTEND) {
      if (x < 0) xi = 0;
    ...
    if (edgeHandling === (Jimp as any).EDGE_WRAP) {
      if (x < 0) {
